...
function transformForHanaWithCsn(inputModel, options) {
 ...
 // check unique constraints - further processing is done in rewriteUniqueConstraints`
  ...
  // Check length of hana entity, column and query identifier
  if(options.toSql && options.toSql.dialect == 'hana' || options.toHana){
    forEachDefinition(csn, checkHanaIdentfierLength);
  }
 ...
 */
  function checkHanaIdentfierLength(artifact, artifactName, prop, path) {
    // entity name is limited to 126 chars for .hdbcds and to 127 for .hdbtable / .sql deployment
    const entityNameLimit = options.toSql ? 127 : 126;
    const maxIdentifierLength = 127;
    if (artifactName.length > entityNameLimit) {
      signalIllegalIdentifier(artifactName, path, 'Entity name', entityNameLimit);
    }

    if(artifact.query){
      forAllQueries(artifact.query, (q, p) => {
        if (q && q.SELECT) {
          // dont be too restrictive with paths, only check query alias and not fully rendered column name
          checkColumnAliasInSelect(q.SELECT, p);
        }
      }, [ 'definitions', artifactName, 'query' ]);
    } else {
      forEachMember(artifact, checkColumnIdentifierLength, [ 'definitions', artifactName ]);
    }

    function checkColumnAliasInSelect(SELECT, path) {
      if (SELECT.columns) {
        SELECT.columns.forEach((column, i) => {
          if (column.as && column.as.length > maxIdentifierLength) {
            signalIllegalIdentifier(column.as, path.concat(['columns', i]), 'Alias');
          }
        });
      }
    }
    
    function checkColumnIdentifierLength(column, columnName, prop, path) {
      if (columnName.length > maxIdentifierLength) {
        signalIllegalIdentifier(columnName, path, 'Column Name');
      }
    }
  
    function signalIllegalIdentifier(identifier, path, kind, maxLen) {
      signal(signal.warning`${kind}: "${identifier}" is too long (${identifier.length}). Maximum Length is ${maxLen ? maxLen : maxIdentifierLength}`, path);
    }
  }
        ...
        if (isManagedAssociationElement(element)) {
          ...
          if (element.keys) {
            ...
            for(let i = 0; i < element.keys.length; i++){
              const key = element.keys[i];
              if(key.ref) {
                key.ref = flattenStructStepsInRef(key.ref, ['definitions', artName, 'elements', elemName, 'keys', i]);
              }
            }
          }
        }
