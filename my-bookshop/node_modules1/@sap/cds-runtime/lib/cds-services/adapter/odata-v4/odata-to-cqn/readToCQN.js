...
const { isSingleton, resolveStructuredName } = require('../utils/handlerUtils')
...
const _orderby = (reflectedEntity, uriInfo, cqn, addKeysToOrder) => {
  orderByToCQN(reflectedEntity, cqn.SELECT, uriInfo.getQueryOption(QueryOptions.ORDERBY), addKeysToOrder)
}
...
const _getPropertyParam = pathSegments => {
  ...
  return pathSegments.length > 1 ? resolveStructuredName(pathSegments, pathSegments.length - 2, [name]).join('.') : name
}
...
const readToCQN = (service, { statements: { SELECT }, target }, odataReq) => {
  ...
  if (isSingleton(reflectedEntity)) {
    cqn.SELECT.one = true
    _orderby(reflectedEntity, uriInfo, cqn, true)
  }

  if (isCollectionOrToMany) {
    _orderby(reflectedEntity, uriInfo, cqn, cqn.SELECT.limit)
  }
