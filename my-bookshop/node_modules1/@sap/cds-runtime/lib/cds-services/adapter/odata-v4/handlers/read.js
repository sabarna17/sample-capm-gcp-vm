...
const { removeContainmentKeys, isSingleton, resolveStructuredName } = require('../utils/handlerUtils')
 ...
 * @private
 */
const _readEntityOrProperty = async (service, req, segments) => {
  let result = await service.dispatch(req)
   ...
   */
  if (result === undefined) {
    if (_isNavigationToOne(segments)) {
 ...
 * @private
 */
const _readCollectionNoCount = async (service, req, odataReq) => {
