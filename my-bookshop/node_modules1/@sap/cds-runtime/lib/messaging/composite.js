...
class Composite extends cds.Service {
  ...
  // REVISIT: Better normalization, overwrite `subscribe` and `dispatch` once
  //          messaging clients overwrite that.
  on (...args) {
    return _delegateAll('on', args, { event: args[0] }, this.normalizedRoutes)
  }
  emit (...args) {
    const event = typeof args[0] === 'object' ? args[0].event : args[0]
