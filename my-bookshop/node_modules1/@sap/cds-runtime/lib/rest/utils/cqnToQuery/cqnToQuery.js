const expressionMap = new Map()
  ...
  .set('expand', (key, value) => {
    return new ExpandExpression(key, value)
  })
...
const _parseColumnsExpression = (colRes, queryOptions) => {
  const resExpand = []
  if (colRes.columns) queryOptions.push(`$select=${colRes.columns}`)
  if (Object.keys(colRes.expand).length !== 0) {
    Object.keys(colRes.expand).forEach(key => {
      if (Object.keys(colRes.expand[key]).length !== 0) {
        resExpand.push(key + _parseColumnsDeepExpand(colRes.expand[key]).join(''))
      } else {
        resExpand.push(key)
      }
    })
    queryOptions.push('$expand=' + resExpand.join(','))
  }
}
...
class ColumnsExpression extends Expression {
  interpret (options) {
    ...
    children.forEach((child, index) => {
      if (children[index + 1] && children[index + 1].constructor.name === 'ExpandExpression') return
      else if (child.constructor.name === 'ExpandExpression') {
        exOutput[children[index - 1].interpret(options)] = child.interpret()
      } else colOutput.push(child.interpret(options))
    })
    return { columns: colOutput.join(','), expand: exOutput }
  }
}

class ExpandExpression extends Expression {
  interpret () {
    if (this.children.length === 1 && this.children[0].constructor.name === 'TerminalExpression') {
      return {}
    }
    if (this.children.length > 0) {
      const columns = new ColumnsExpression('columns', this.value)
      columns.children = this.children
      return columns.interpret()
    }
    return {}
  }
}
    ...
    this.children.forEach(child => {
      switch (child.constructor.name) {
        ...
        case 'ColumnsExpression':
          _parseColumnsExpression(child.interpret(options), queryOptions)
          break
