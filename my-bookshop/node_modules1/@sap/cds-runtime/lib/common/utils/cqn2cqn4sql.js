...
const _flattenProps = (subElement, structName, structProperties, structElement, flattenedElements, model) => {
  if (subElement.kind === 'type') {
    _resolveStructured(
      {
    ...
    flattenedElements.push({ ref: [`${structName}_${structElement}`] })
  }
}

const _resolveStructured = ({ structName, structProperties }, subElements, model, flattenedElements) => {
  if (!subElements) {
    return
  }
  ...
  for (const structElement in subElements) {
    _flattenProps(subElements[structElement], structName, structProperties, structElement, flattenedElements, model)
  }
}
...
const _flattenStructuredForExpand = (column, expandedEntity, model) => {
  ...
  for (const expandElement of column.expand) {
    ...
    if (element && element.kind === 'type') {
      ...
      _resolveStructured({ structName: propertyName, structProperties: [] }, element.elements, model, flattenedElements)
    }
  }
...
const _flatten = (SELECT, model, flattenedElements, toBeDeleted) => {
  ...
  for (const entityName of entityNames) {
    ...
    for (const column of SELECT.columns) {
      ...
      if (column.expand) {
        _flattenStructuredForExpand(column, csnEntity.elements[structName]._target, model)
        continue
      }
      ...
      if (element && element.kind === 'type') {
        ...
        _resolveStructured(
          { structName, structProperties: column.ref.slice(1) },
          element.elements,
          model,
          flattenedElements
        )
...
const _flattenStructured = (SELECT, model) => {
  if (Array.isArray(SELECT.columns) && SELECT.columns.length > 0) {
    ...
    SELECT.columns = SELECT.columns.filter(e => (e.ref && !toBeDeleted.includes(e.ref[0])) || e.func) // TODO aliases?
